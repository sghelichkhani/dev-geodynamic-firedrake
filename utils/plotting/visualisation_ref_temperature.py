...
output_path = './PNGs/rectangle_extruded_Q2/FWD_refmodel'

# create a new 'PVD Reader'
temperaturepvd = PVDReader(registrationName='temperature.pvd', FileName='/home/sia/Workplace/dev-geodynamic-firedrake/adjoint_tic/rectangle_Q2/FWDREFmodel/temperature.pvd')
temperaturepvd.CellArrays = []
temperaturepvd.PointArrays = ['Temperature']
...
# set scalar coloring
ColorBy(temperaturepvdDisplay, ('POINTS', 'Temperature'))
...
viscosityLUT = GetColorTransferFunction('Temperature')
...
viscosityPWF = GetOpacityTransferFunction('Temperature')
...
viscosityLUTColorBar.Title = 'Temperature'
...
viscosityLUT.RescaleTransferFunction(0.25, 0.55)

# Rescale transfer function
viscosityPWF.RescaleTransferFunction(0.25, 0.55)
...
for i in range(int(animationScene1.EndTime+1)):
    ...
    SaveScreenshot(os.path.join(output_path, str('fig_%3.3i.png' %k)), renderView1, ImageResolution=[1800, 2000],
