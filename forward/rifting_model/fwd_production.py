...
blb_ctr_h = as_vector((0.5, 0.65)) 
blb_gaus = Constant(0.10)

# A linear temperature profile from the surface to the CMB, with a gaussian blob somewhere
#T_old.interpolate( 1.0 - y + 0.3*exp(-0.5*((X-blb_ctr_h)/blb_gaus)**2));
T_old.interpolate( 1.0 - y);
...
# Setting free-slip BC for top and bottom
u_top = Function(V, name='topVelocity')
u_top.interpolate(1e8*as_vector((+tanh((x-0.5)/0.05), 0.0)))

bcu_top         = DirichletBC(Z.sub(0), u_top, (top_id))
bcu_base        = DirichletBC(Z.sub(0).sub(0), 0.0, (bottom_id))
bcu_rightleft   = DirichletBC(Z.sub(0).sub(1), 0.0, (left_id, right_id))
...
stokes_problem = LinearVariationalProblem(a, L, z, constant_jacobian=False, bcs=[bcu_top, bcu_base, bcu_rightleft])
...
energy_problem = LinearVariationalProblem(a_energy, L_energy, T_new, constant_jacobian=False, bcs=[bct_base, bct_top])
