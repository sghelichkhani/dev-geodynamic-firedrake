...
Y_SUPG   = Y + dot(u,grad(Y)) * (h / (2*sqrt(dot(u,u))))
F_energy = Y_SUPG * ((T_new - T_old) / delta_t) * dx + Y_SUPG*dot(u,grad(T_theta)) * dx + dot(grad(Y),kappa*grad(T_theta)) * dx

# Write output files in VTK format:
u_file = File('FWDREFmodelSUPG/velocity.pvd')
p_file = File('FWDREFmodelSUPG/pressure.pvd')
t_file = File('FWDREFmodelSUPG/temperature.pvd')
...
checkpoint_data = DumbCheckpoint("final_state_supg", single_file=True, mode=FILE_CREATE, comm=mesh.comm)
