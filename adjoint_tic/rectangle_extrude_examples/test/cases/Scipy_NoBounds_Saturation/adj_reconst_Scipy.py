...
class myReducedFunctional(ReducedFunctional):
    def __init__(self, functional, controls, scale=1.0, tape=None,
        ...
        self.cntrl = Function(T_ic.function_space(), name="control")

    def __call__(self, values):
        func_value = super().__call__([val.project(conditional(gt(val, T_ub), T_ub, conditional(lt(val, T_lb), T_lb, val))) for val in values])*1e4
        return func_value
