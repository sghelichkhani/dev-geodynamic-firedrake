...
u_top = Function(V, name='ReferenceVelocity')
bcu_topbase     = DirichletBC(Z.sub(0).sub(1), 0.0, (top_id, bottom_id)) # top and bottom boundary is always free-slip
...
chckpnt_u = DumbCheckpoint("../../Velocity",mode=FILE_READ)

u_misfit = 0

# Now perform the time loop:
for timestep in range(0, max_num_timesteps):
    # Checking timestep and time 
    chckpnt_u.set_timestep(t=simu_time, idx=timestep)
    chckpnt_u.load(u_top, name='Velocity')

    # Solve system - configured for solving non-linear systems, where everything is on the LHS (as above)
    # and the RHS == 0. 
    stokes_solver.solve()

    u_misfit += assemble(0.5 * dot(u - u_top, u - u_top) * ds_t)
...
T_mag = assemble(0.5*(final_state)**2 * dx)
u_mag = assemble(0.5*(u_top)**2 * ds_t)
## Initialise functional
T_misfit = assemble(0.5*(T_new - final_state)**2 * dx)
functional =  u_misfit/(u_mag*max_num_timesteps)*5e-6 + T_misfit/T_mag
