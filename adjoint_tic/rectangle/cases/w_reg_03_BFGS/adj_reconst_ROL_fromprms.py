...
beta_r = 1e-4/ assemble(inner(grad(final_state - T_mean), grad(final_state - T_mean)) * dx)
...
class OptimisationOutputCallbackPost:
    ...
    def __call__(self, cb_functional, dj, controls):
        ...
        func_val = assemble(beta_m*(self.T_tc_copy-final_state)**2 * dx)
        reg_val  = assemble(beta_r*inner(grad(self.T_ic_copy-T_mean), grad(self.T_ic_copy - T_mean)) * dx)
        grad_val = assemble((self.grad_copy)**2 * dx)

        log('# Der {}, ||Misfit|| {}, ||Regu|| {}, ||Grad|| {}'.format(self.iter_idx, func_val, reg_val, grad_val))
