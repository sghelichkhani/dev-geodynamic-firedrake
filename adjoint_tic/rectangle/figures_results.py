...
main_path = '/Users/sghelichkhani/Data/cosgrove/Workplace/dev-geodynamic-firedrake/adjoint_tic/rectangle/cases'
...
for i, case in enumerate(ll_dirs):
    ...
    try:
        ...
        iters[i], fvals[i], grads[i], nfvals[i], ngrads[i] =\
                    read_output_ROL(glob.glob(os.path.join(case,'*.dat'))[0],\
...
for iteration in range(50, 51, 2):
    plt.close(1)
    ...
    for i in ax.get_children():
        if isinstance(i, matplotlib.spines.Spine):
            i.set_color('red')
            i.set_linewidth(4)
    ax.text(0.50, 0.95, str('Iteration %3.1i' %iteration), rotation=0, style='italic', ha='center', va='center', fontsize=32, transform=fig.transFigure,
                bbox={'facecolor':(0.9, 0.9, 0.9), 'alpha':1.0, 'lw':2.0,'pad':4,'edgecolor':'k'})
    show_image('./PNGs/00_FWD_refmodel/fig_00.png', ax)
    bx = []
    for i, fi_name in enumerate(list(captions.keys())):
        ...
        show_image(os.path.join('./PNGs',fi_name,str('fig_init_%2.2i.png') %ngrads[i][abs(np.array(iters[i])-iteration+1).argmin()]), bx[-1])
        bx[-1].text(0.50, 0.95, captions[list(captions.keys())[i]], rotation=0, style='italic', ha='center', va='center', fontsize=12, transform=bx[-1].transAxes,\
                bbox={'facecolor':(0.9, 0.9, 0.9), 'alpha':1.0, 'lw':2.0,'pad':4,'edgecolor':'k'})
        bx[-1].set_axis_off()
    fig_name=str('PNGs/initial_iter%3.3i.png'  %iteration)
    print(fig_name)
