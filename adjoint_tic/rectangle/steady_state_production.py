"""
    Generation of a steady state profile 
"""
...
def compute_timestep(u):
    ...
    return max(ts_min*target_cfl_no,max_timestep)
...
t_file = File('SteadyState/temperature.pvd')
t_file.write(T_new)
...
for timestep in range(0, max_num_timesteps):
    ...
    # and the RHS == 0. 
    solve(F_stokes==0, z, bcs=[bcu_topbase, bcu_rightleft], solver_parameters=stokes_solver_parameters)
    ...
    # Calculate the L2-norm of change in temperature
...
# Write output temperature in VTK format:
t_file.write(T_new)



# Generating the reference temperature field for the adjoint
checkpoint_data = DumbCheckpoint("steady_state", single_file=True, mode=FILE_CREATE, comm=mesh.comm)
