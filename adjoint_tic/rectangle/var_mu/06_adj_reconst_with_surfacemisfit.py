...
# ********************** Setup Equations ************************ 
# viscosiy 
mu        = exp(-ln(10)*T_theta) # Constant viscosity
...
for timestep in range(0, max_num_timesteps):
    ...
    velocity_mag  = assemble(0.5 * dot(u_top, u_top) * ds(top_id))
    functional += assemble( 0.5/velocity_mag * dot(u-u_top, u-u_top) * dx)
...
temperature_mag = assemble(0.5 * final_state**2 * dx)

## Initialise functional
functional += assemble(0.5/temperature_mag * (T_new - final_state)**2 * dx)
...
class OptimisationOutputCallbackPost:
    def __init__(self):
        self.iter_idx = 0
        self.opt_file             = File('./outputs/05_Reconstruction_surface_misfit/opt_file.pvd') 
    ...
    def __call__(self, cb_functional, dj, controls):
        ...
        log('# Der {}, ||Misfit|| {}, ||Grad|| {}'.format(self.iter_idx, func_val, grad_val))
