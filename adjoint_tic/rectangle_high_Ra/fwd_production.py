...
def compute_timestep(u):
    ...
    return 5e-6 # I am changing it to constant time-step for now 
...
T_old.interpolate(0.5 - 0.1*exp(-0.5*((X-blb_ctr_h)/blb_gaus)**2));
...
log("Simulation finished:", simu_time, timestep)
u_file.write(u)
p_file.write(p)
t_file.write(T_new)
