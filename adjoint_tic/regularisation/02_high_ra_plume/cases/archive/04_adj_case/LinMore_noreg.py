...
dim_misfit = assemble(final_state**2 * dx)
dim_regterm = assemble(dot(grad(final_state), grad(final_state)) * dx)

## Initialise functional
misfit = assemble(0.5*(T_new - final_state)**2 * dx)
regterm =  dim_misfit/dim_regterm*assemble(0.5*(dot(grad(T_ic), grad(T_ic)) * dx))

# total functional
functional =  misfit + 1e-2*regterm
