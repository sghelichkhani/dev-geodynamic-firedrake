...
bcu_top_right_left = DirichletBC(Z.sub(0), u_surface, (top_id, left_id, right_id))
bcu_base = DirichletBC(Z.sub(0).sub(1), 0.0, (bottom_id))
all_u_bounds = [bcu_top_right_left, bcu_base]
...
class myStatusTest(ROL.StatusTest):
    def __init__(self, params, vector):
        ...
        self.opt_t_final_file = File('visual_17_imposed_allthree/opt_temperature_fin.pvd')
        self.opt_t_init_file = File('visual_17_imposed_allthree/opt_temperature_int.pvd')

        self.solution_checkpoint = CheckpointFile("solution_17_imposed_allthree.h5",
                                                  mode="w"
                                                  )
...
params = {
        'General': {
              'Print Verbosity': 1,
              'Output Level': 3,
              'Krylov': {  # These are needed for our solution
                           # of the hessian I guess
                    "Iteration Limit": 10,
