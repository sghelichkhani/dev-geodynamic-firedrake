...
regularisation = assemble(alpha * 0.5*(dot(grad(T_ic), grad(T_ic)))**2 * dx)
...
class myStatusTest(ROL.StatusTest):
    def __init__(self, params, vector):
        ...
        self.opt_t_final_file    = File(str('visual_beta_%5.4e_%3.3i/opt_temperature_fin.pvd' %(alpha, max_num_timesteps)))
        self.opt_t_init_file     = File(str('visual_beta_%5.4e_%3.3i/opt_temperature_int.pvd' %(alpha, max_num_timesteps)))
