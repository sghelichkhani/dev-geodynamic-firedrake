...
with CheckpointFile("State_SteadyState.h5", "r") as t_ic_checkpoint:
    ...
    final_state = t_ic_checkpoint.load_function(mesh, "Temperature")
...
def beta(Pe):
    """Component-wise beta formula Donea and Huerta
     (2.47a) for SU stabilisation"""
...
stokes_solver = NonlinearVariationalSolver(stokes_problem,
                                           solver_parameters=stokes_iterative,
                                           appctx={"mu": mu},
...
energy_solver = NonlinearVariationalSolver(energy_problem,
                                           solver_parameters=energy_iterative
                                           )
...
class myStatusTest(ROL.StatusTest):
    def __init__(self, params, vector):
        ...
        self.opt_t_final_file = File(str('visual_%3.3i/opt_temperature_fin.pvd' %max_num_timesteps))
        self.opt_t_init_file = File(str('visual_%3.3i/opt_temperature_int.pvd' %max_num_timesteps))

        # A linear temperature profile from the surface to 
        # the CMB, with a gaussian blob somewhere
