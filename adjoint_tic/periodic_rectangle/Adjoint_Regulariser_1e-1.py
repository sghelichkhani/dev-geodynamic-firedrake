...
functional = assemble(0.5 * (T_new - final_state)**2 * dx) +\
            1e-1*final_state_mag/final_state_grad_mag*assemble(0.5 * dot(grad(T_ic-T_ave), grad(T_ic-T_ave)) * dx)
...
class OptimisationOutputCallbackPost:
    def __init__(self):
        self.iter_idx = 0
        self.opt_file             = File('Adjoint_Regular1e-1/opt_file.pvd')
...
#my_field = reduced_functional.derivative({'riesz_representation':'L2'})
#fi = File('diff.pvd')
#fi.write(my_field)
...
#        self.opt_t_final_file    = File('AdjVis/opt_temperature_final.pvd')
#        self.opt_t_init_file     = File('AdjVis/opt_temperature_initial.pvd')
#
#    @no_annotations
